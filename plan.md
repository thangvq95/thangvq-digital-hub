...
|---|---|---|
| Framework | **Next.js 14+ (App Router)** | SSR/SSG cho SEO portfolio, RSC cho dashboard |
| Styling | **Tailwind CSS v4** + **ShadcnUI** | Rapid UI, consistent design system |
| Database | **Supabase (PostgreSQL)** | Free tier đủ dùng, realtime, REST API sẵn |
| Crawler | **OpenClaw** (external) | Scrape + LLM classify + gọi API để upsert |
| Hosting | **Vercel** | Edge deployment, tích hợp tốt với Next.js |
| Domain | `thangvq95.page` | Đã có sẵn |
...
sequenceDiagram
    ...
    OC->>OC: LLM classify domains cho mỗi repo mới
    OC->>API: POST /api/repos/upsert (batch payload + API key)
    API->>API: Validate API key
    API->>DB: Reset rank_* = NULL (theo period)
    API->>DB: Upsert repositories (preserve is_favorite, is_applied)
    API->>DB: Insert sync_log
    API-->>OC: 200 OK + sync summary
```

> **Lưu ý:** OpenClaw chịu trách nhiệm hoàn toàn việc scrape và classify.
> App chỉ cung cấp API endpoint để nhận dữ liệu đã xử lý và lưu vào Supabase.
...
> **Nguồn:** LinkedIn [thangvq95](https://www.linkedin.com/in/thangvq95/) — sẽ được cập nhật chi tiết sau.
...
- Hard-working, quick-learning, creative — luôn tìm kiếm môi trường thử thách
- Flutter as primary focus: production apps cho multiple startups
...
> 📝 *Chi tiết descriptions sẽ được bổ sung sau khi user cập nhật CV*
...
- Projects: Sổ Giáo Dân platform, Care app, Rovo app, và personal infra setup
- *Screenshots và chi tiết sẽ được bổ sung*
...
#### Read APIs (Dashboard frontend dùng)
...
#### Write API (OpenClaw gọi để insert data)

| Endpoint | Method | Auth | Description |
|---|---|---|---|
| `/api/repos/upsert` | POST | `x-api-key` header | Nhận batch data từ OpenClaw |
...
- [ ] Deploy to Vercel
- [ ] Domain DNS setup (`thangvq95.page` → Vercel)
...
1. **Monorepo vs Multi-repo:** Single Next.js app — portfolio và dashboard share layout, fonts, theme
2. **SSR vs SSG:** Portfolio pages dùng SSG (static), Dashboard dùng SSR + client-side fetching
3. **Supabase vs Self-hosted PG:** Supabase cho tiện — free tier đủ cho side project, có REST API sẵn
4. **OpenClaw integration:** OpenClaw scrape + LLM classify hoàn toàn bên ngoài, app chỉ nhận data qua API endpoint. Không cần LLM key trong app.
5. **RLS Policy:** Bật Row Level Security — anon key chỉ đọc, service role key cho upsert API
6. **Hosting:** Vercel — tích hợp native với Next.js, deploy nhanh, preview deployments
7. **Portfolio content:** Tạm dùng data từ LinkedIn, sẽ cập nhật chi tiết sau khi user hoàn thiện CV
