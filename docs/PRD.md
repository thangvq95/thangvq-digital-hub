...
graph TB
    ...
    subgraph "Stateful Repo & Backend (VPS / GitHub)"
        GP["GitHub Repo & Hermes Kanban (Task/DAG state)"]
...
| Project Management | **Hermes Kanban** | Single Source of Truth for Task/DAG state (`kanban.thangvq95.page`) |
...
graph TB
    ...
    L2 -->|Hermes executes| L3["3. Runtime State (Hermes Kanban/Postgres)"]
...
| **3. Runtime State** | Execution state, Task DAG tracking, persistent memory | Hermes Kanban + PostgreSQL |
| **4. Documentation** | Human-facing architecture, core truth | GitHub (`docs/PRD.md`, `docs/architecture/*`) |

### Execution Pipeline

```
Brainstorm (brainstorming skill) → Plan (writing-plans skill) → Hermes executes → Hermes Kanban tracking → docs update
```
...
4. **Track** — Task status synced to Hermes Kanban
...
| Component | Role |
|---|---|
| **Hermes Agent** | Remote Developer / Autonomous Worker — executes cronjobs, heavy tasks, and manages its own DAG flows directly on the VPS. |
| **Local Agents** | Cursor / Claude Code — stateless local development interfaces. Synchronize completely via GitHub. |
| **GitNexus** | Knowledge graph — provides unified codebase context to all agents via MCP. Hosted on VPS. |
| **Playwright** | Testing — E2E & API automation. |
| **Hermes Kanban** | Primary task and DAG state orchestrator (`kanban.thangvq95.page`). |
...
3. **No Manual Sync** — Hermes syncs task state between specs and Hermes Kanban
...
- **VPS (Remote Execution):** Runs Dockerized Backend (NestJS), Database (PostgreSQL), and Hermes Agent (Remote Worker).
