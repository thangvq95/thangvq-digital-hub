...
> **Execution authority:** Implementation Plan (generated by `writing-plans` skill).

---

## Execution Authority

Hermes executes tasks based on the Implementation Plan. The flow:

```
Brainstorm RFC (docs/superpowers/specs/*) → reference only
        ↓
`writing-plans` generates Implementation Plan → execution sequence
        ↓
Hermes executes tasks → runtime state in GitHub Projects
```
