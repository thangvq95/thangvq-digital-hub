      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-24 space-y-24">
        ...
        <section id="stack-grid" className="scroll-mt-20">
          <div className="flex items-center gap-3 mb-8">
            <div
              ...
              style={{ background: "#60A5FA", boxShadow: "0 0 12px #60A5FA80" }}
            />
            <div>
              <h2
            ...
            {[
              {
                label: "Traffic Flow",
                detail:
                  "User → Cloudflare WAF → Vercel (SSR/SSG) → NestJS API → PostgreSQL",
                color: "#F59E0B",
              },
              {
                label: "AI Layer",
                detail:
                  "Hermes Agent on VPS ↔ GitHub (sync) + 9Router (LLM) + GitNexus (context)",
                color: "#A78BFA",
              },
              {
                label: "Containerized",
              ...
              },
              {
                label: "No Open Ports",
                detail:
                  "All VPS traffic routed via Cloudflare Tunnel — zero exposed ports",
                color: "#F59E0B",
              },
              {
                label: "Persistent Brains",
                detail:
                  "Docker Named Volumes: hermes_data + gitnexus_data — survives container updates",
                color: "#A78BFA",
              },
            ].map((item) => (
              <div
          ...
          <div className="space-y-3">
            {[
              ...
              },
              {
                ...
                detail:
                  "0 10 * * * — Check favorites → GitHub Releases API → has_new_release=true",
                color: "#34D399",
              },
              {
                label: "🔧 Cron-Polled Fix",
                detail:
                  "gh issue/pr list → SQLite dedup → git worktree → hermes skill → push PR",
                color: "#A78BFA",
              },
              {
              ...
              },
              {
                ...
                detail:
                  "All LLM calls go to 9router.phieucaphe.com/v1. No local models on VPS.",
                color: "#F59E0B",
              },
            ].map((item) => (
              <div
            ...
            {[
              {
                ...
                detail:
                  "lint-staged runs ESLint + Prettier. commitlint enforces Conventional Commits format.",
                color: "#F472B6",
              },
              {
                label: "Merge to Main",
                detail:
                  "SSH → VPS → docker compose pull → health check /api/repos → deploy via Vercel CLI.",
                color: "#60A5FA",
              },
              {
                label: "Backend-First Strategy",
                detail:
                  "Frontend deploys only AFTER backend health check passes. Eliminates version mismatches.",
                color: "#22C55E",
              },
              {
                label: "release-please Bot",
                detail:
                  "Analyzes commits on main → bumps semver → auto-generates CHANGELOG.md → creates GitHub Release tag.",
                color: "#A78BFA",
              },
              {
                label: "Playwright E2E",
                detail:
                  "Tests run in CI against the live dev server. No green tests = no merge.",
                color: "#22C55E",
              },
            ].map((item) => (
              <div
          ...
          <div className="space-y-3">
            {[
              ...
              },
              {
                ...
                detail:
                  "For each is_favorite=true repo: GET /releases/latest → compare tag_name → if changed: has_new_release=true.",
                color: "#34D399",
              },
              {
                label: "Field-Level Update Strategy",
                detail:
                  "Dynamic fields (stars, forks, description) always updated. User fields (is_favorite, ai_summary) never overwritten.",
                color: "#F59E0B",
              },
              {
                label: "✨ Magic Analyze (on-demand)",
                detail:
                  "User clicks button → POST /api/repos/{name}/analyze → Hermes reads README → Markdown summary → cached forever.",
                color: "#A78BFA",
              },
            ].map((item) => (
              <div
            ...
            {[
              {
                ...
                detail:
                  "NestJS Global Exception Filter sends all unhandled errors to Sentry with full stack trace + context.",
                color: "#EF4444",
              },
              {
                label: "🔗 Webhook → GitHub Issue",
                detail:
                  "POST /webhooks/sentry → validate HMAC → create GitHub Issue with Sentry error details automatically.",
                color: "#FB923C",
              },
              {
                label: "🤖 Hermes Picks Up",
                detail:
                  "Listens for GitHub issue_opened events → runs diagnose + tdd skills → writes fix → pushes branch.",
                color: "#A78BFA",
              },
              {
                label: "✅ Auto PR + Close",
                detail:
                  "gh pr create --fill → CI passes → PR merged → issue auto-closed. Full loop with zero human input.",
                color: "#22C55E",
              },
            ].map((item) => (
              <div
        ...
        >
          ...
          <p className="text-text-secondary mb-6 max-w-lg mx-auto">
            All architecture decisions, agent rules, and skill definitions are
