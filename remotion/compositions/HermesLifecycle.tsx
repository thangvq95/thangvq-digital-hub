...
const STEP_H = 70;
const steps = [
  { x: 60,  label: 'GitHub Webhook', sub: 'push / PR / check_run', color: COLORS.textSecondary },
  { x: 240, label: 'listener.py', sub: 'port 8080', color: COLORS.blue },
  { x: 420, label: 'HMAC Verify', sub: '+ SQLite dedup', color: COLORS.amber },
  { x: 600, label: 'git worktree', sub: 'isolated workspace', color: COLORS.purple },
  { x: 780, label: 'Hermes Agent', sub: 'hermes -s <skill>', color: COLORS.purple },
  { x: 960, label: 'Playwright', sub: 'tests pass ✓', color: COLORS.accent },
  { x: 1100,label: 'git push + PR', sub: 'back to branch', color: COLORS.accent },
];
  ...
  return (
    <AbsoluteFill style={{ background: COLORS.bg }}>
      ...
      <svg style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', opacity: 0.05 }}>
        <defs>
          <pattern id="grid2" width="40" height="40" patternUnits="userSpaceOnUse">
            <path d="M 40 0 L 0 0 0 40" fill="none" stroke={COLORS.purple} strokeWidth="0.5" />
          </pattern>
        </defs>
      ...
      <FadeIn from={T.pipeStart} durationFrames={15}>
        <div style={{ position: 'absolute', top: 200, left: 60, fontFamily: FONTS.body, fontSize: 12, color: COLORS.textMuted, letterSpacing: 2, textTransform: 'uppercase' }}>
      ...
      {steps.map((step, i) => {
        ...
        return (
          <React.Fragment key={i}>
            <GlassCard
              x={step.x} y={STEPS_Y} width={STEP_W} height={STEP_H}
              appearFrame={appearFrame}
              accentColor={step.color}
            >
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
                <span style={{ fontFamily: FONTS.heading, fontSize: 12, fontWeight: 700, color: COLORS.textPrimary, textAlign: 'center' }}>
                  {step.label}
                </span>
        ...
        <FadeIn from={T.cronSection} durationFrames={20}>
          <div style={{ position: 'absolute', bottom: 40, left: 60, right: 60, display: 'flex', gap: 16 }}>
            {[
              ...
              { time: '0 10 * * *', label: 'Release Monitor', sub: 'Check favorites → POST /api/repos/check-releases', color: COLORS.accent },
            ].map((cron, i) => (
              <div key={i} style={{
                ...
                <div style={{ fontFamily: FONTS.mono, fontSize: 11, color: cron.color, marginBottom: 4 }}>
                  ⏰ {cron.time}
                </div>
                <div style={{ fontFamily: FONTS.heading, fontSize: 13, fontWeight: 600, color: COLORS.textPrimary }}>
                  {cron.label}
                </div>
                <div style={{ fontFamily: FONTS.body, fontSize: 11, color: COLORS.textSecondary, marginTop: 2 }}>
                  {cron.sub}
                </div>
              </div>
            ))}
